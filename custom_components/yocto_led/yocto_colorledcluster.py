...
class YColorLedCluster(YFunction):
    ...
    def set_rgbColor(self, ledIndex, count, rgbValue):
        ...
        return self.sendCommand(
            "SR" + str(int(ledIndex)) + "," + str(int(count)) + "," + ("%x" % rgbValue)
        )
    ...
    def set_rgbColorAtPowerOn(self, ledIndex, count, rgbValue):
        ...
        """
        return self.sendCommand(
            "SC" + str(int(ledIndex)) + "," + str(int(count)) + "," + ("%x" % rgbValue)
        )
    ...
    def set_hslColor(self, ledIndex, count, hslValue):
        ...
        """
        return self.sendCommand(
            "SH" + str(int(ledIndex)) + "," + str(int(count)) + "," + ("%x" % hslValue)
        )
    ...
    def addRgbMoveToBlinkSeq(self, seqIndex, rgbValue, delay):
        ...
        """
        return self.sendCommand(
            "AR" + str(int(seqIndex)) + "," + ("%x" % rgbValue) + "," + str(int(delay))
        )
    ...
    def addHslMoveToBlinkSeq(self, seqIndex, hslValue, delay):
        ...
        """
        return self.sendCommand(
            "AH" + str(int(seqIndex)) + "," + ("%x" % hslValue) + "," + str(int(delay))
        )
    ...
    def addJumpToBlinkSeq(self, seqIndex, linkSeqIndex):
        ...
        """
        return self.sendCommand(
            "AC" + str(int(seqIndex)) + ",100," + str(int(linkSeqIndex)) + ",1000"
        )
    ...
    def get_linkedSeqArray(self, ledIndex, count):
        ...
        buff = self._download(
            "rgb.bin?typ=1&pos=" + str(int(ledIndex)) + "&len=" + str(int(count))
        )
        del res[:]
    ...
    def get_blinkSeqStateSpeed(self, seqIndex, count):
        ...
        buff = self._download(
            "rgb.bin?typ=6&pos=" + str(int(seqIndex)) + "&len=" + str(int(count))
        )
        del res[:]
    ...
    def get_blinkSeqStateAtPowerOn(self, seqIndex, count):
        ...
        buff = self._download(
            "rgb.bin?typ=5&pos=" + str(int(seqIndex)) + "&len=" + str(int(count))
        )
        del res[:]
    ...
    def get_blinkSeqState(self, seqIndex, count):
        ...
        buff = self._download(
            "rgb.bin?typ=3&pos=" + str(int(seqIndex)) + "&len=" + str(int(count))
        )
        del res[:]
    ...
    def FirstColorLedCluster():
        ...
        p = (ctypes.c_int * 1)()
        err = YAPI.apiGetFunctionsByClass(
            "ColorLedCluster", 0, p, size, neededsizeRef, errmsgRef
        )
        ...
        if YAPI.YISERR(
            YAPI.yapiGetFunctionInfo(
                p[0], devRef, serialRef, funcIdRef, funcNameRef, funcValRef, errmsgRef
            )
        ):
            return None

        return YColorLedCluster.FindColorLedCluster(
            serialRef.value + "." + funcIdRef.value
        )
